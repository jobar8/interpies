...
class grid(object):
    ...
    def info(self):
        ...
        print('Number of null cells: {} ({:.2f}%)'.format(nanCells, 100*nanCells/(self.nrows * self.ncols)))
        # stats
    ...
    def dx(self, method='SG', deg=3, win=5, doEdges=True, fs_tap=5, **kwargs):
        ...
        if method.upper() == 'SG':
            ...
            output = transforms.fourier_transformform(self.data, self.cellsize,
                                              trans='dx', **kwargs)
        else:
            raise ValueError('Method {} has not been recognised.'.format(method))
